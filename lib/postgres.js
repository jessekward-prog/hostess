...
async function provisionDatabase(appName, existingPassword) {
  ...
  return {
    network: NETWORK,
    rolePassword,
    // sslmode=disable: this container doesn't speak TLS, and apps that force SSL
    // whenever NODE_ENV=production (a common pattern) need the connection string
    // itself to say so, since they don't otherwise know this Postgres is local.
    databaseUrl: `postgres://${roleName}:${rolePassword}@${PG_CONTAINER}:5432/${dbName}?sslmode=disable`,
