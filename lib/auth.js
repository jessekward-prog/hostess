...
function lockStatus() {
  ...
  return remaining > 0 ? { locked: true, retryAfterMs: remaining } : { locked: false };
}
...
function verifyPin(req, res, candidate) {
  ...
  if (!pin || candidate !== pin) {
    failedAttempts += 1;
    if (failedAttempts >= 5) {
      lockedUntil = Date.now() + Math.min(30000 * (failedAttempts - 4), 10 * 60 * 1000);
    }
    return false;
  }
  failedAttempts = 0;
...
module.exports = { middleware, rotatePin, clearPin, getPin, verifyPin, lockStatus, isTunneled, COOKIE_NAME };
